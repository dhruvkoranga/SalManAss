...
- [x] Employee filter options endpoint (`GET /api/employees/filters`) — distinct
      country/department/role values, backs the frontend's autocomplete suggestions
- [x] Input validation (salary, currency, required fields)
- [x] Unit tests — backend core logic (46 tests passing)
...
- [x] Autocomplete suggestions on Country/Department/Role filters, sourced from
      the live data (not a hardcoded list, since these fields are editable free text)
- [x] Single employee view/edit
- [x] Analytics view (avg/median by country/department/role, salary distribution)
- [x] Unit tests — frontend components (16 tests passing)
